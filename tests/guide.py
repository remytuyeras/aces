...
s = \
f"""\033[94m>>>\033[0m repartition.q
{repr(repartition.q)}
...
s = \
f"""\033[94m>>>\033[0m x = np.array(ac.x); x \033[92m# tuple of polynomials\033[0m
{repr(x)}
...
public = ac.publish()
s = \
f"""\033[94m>>>\033[0m public = ac.publish()
...
print(s,end="")
ac = pyc.ArithChannel(p=32,N=10,deg_u=3,repartition=repartition,
                      anchor=lambda _: list(range(6))
                      )
public = ac.publish()

s = \
f"""\033[94m>>>\033[0m public["levels"]
{public['levels']}
...
s = \
f"""\033[94m>>>\033[0m m1 = random.randrange(ac.p); m1
{repr(m1)}
...
try:
    ...
    error = e
s = \
f"""\033[90mTraceback (most recent call last):
...
s = \
f"""\033[94m>>>\033[0m equation_schema = "0*1+2*5+3*4+6*7+1*5"
...
s = \
f"""\033[94m>>>\033[0m alice.decrypt(cipher_output)
...
s = \
f"""\033[94m>>>\033[0m a = alg.add(cip1, cip2)
...
diff_dec = [a.dec[i]-b.dec[i] for i in range(ac.n)]
s = \
f"""\033[94m>>>\033[0m a = alg.mult(cip1, cip2)
...
diff_dec = [a.dec[i]-b.dec[i] for i in range(ac.n)]
s = \
f"""\033[94m>>>\033[0m a = alg.add(alg.add(cip1, cip2), cip3)
...
s = \
f"""\033[94m>>>\033[0m a = alg.mult(alg.mult(cip1, cip2), cip3)
