...
def test_repartition():
    ...
    repartition = pyc.Repartition(n=5, p=2, upperbound=47601551)  # Initialize with parameters
    ...
    print("sigma_img =", repartition.sigma_img)
    print("sigma_fibers =", repartition.sigma_fibers)
    assert len(repartition.sigma_fibers[0]) == len(repartition.sigma_img)  # Check lengths match
    ...
    print("\nIs the sum (\\sum_k x_k(omega) * q_{\\sigma(k)} * mu_k) equal to 1?", gcd_is_one)
    bezout_sum = 0
    for k in range(repartition.n):
        bezout_sum += repartition.factors[repartition.sigma_img[k]] * repartition.mus[k] * repartition.x_images[k]

    print(
        "We can verify that \\sum_k x_k(omega) * q_{\\sigma(k)} * mu_k =", 
        bezout_sum % repartition.q, ", which is " + ("indeed" if gcd_is_one else "not")+" 1"
        )

    assert (bezout_sum % repartition.q == 1) or not(gcd_is_one), "Error in construct_mus()"
    ...
    for i in range(repartition.n):
        for j in range(repartition.n):
            result = repartition.x_images[i] * repartition.x_images[j] - repartition.ell[i][j] * repartition.sigma_q(i, j)
            linear_i_j = 0
            for k in range(repartition.n):
            ...
            result % repartition.q, "|", linear_i_j % repartition.q,
            f"= \t\\sum_k x_k(omega) \\lambda_{{{i,j}}}^k"
            )

            assert result % repartition.q == linear_i_j % repartition.q, "Error in construct()"
