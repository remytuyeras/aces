...
```
To do arithmetic on encrypted information, use the class ```ACESAlgebra```. The decryption of the cyphertexts can be done at any point by using the class ```ACESReader```.  For example, we have the following behaviour (where the object ```ac.x``` refers to the private key $x$ mentioned in the research paper).
```python
...
15
```
A canonical attack for the ACES cryptosystem consists in evaluating the encryption polynomial at $\omega=1$. For most parameterizations, we can verify that this attack collects what empirically appears to be a random distribution.
