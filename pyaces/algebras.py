...
class ACESAlgebra(Algebra):
    ...
    def __init__(self, p: int, q: int, n: int, u: Polynomial,
                tensor: list[list[list[int]]], debug: bool = False,
    ...
    def add(self, a: ACESCipher, b: ACESCipher, refresh: bool = True) -> ACESCipher:
        ...
        if noise / total_noise >= .99:

            if self.debug:
                print(f"\033[92mACESAlgebra.add: next operation will reach max. saturation, at {round(100 * noise / total_noise, 4)}% of max. noise level.\033[0m")

            if refresh and self.refresh_classifier is not None and self.encrypter is not None and self.refresher is not None:
                
                # Refresh `a` if needed
                estimated_refresh_noise = self.assess_refresh_level(refresher=self.refresher, corefresher=a.corefresher(self.encrypter))
                if  estimated_refresh_noise >= a.lvl:

                    if self.debug:
                        print(f"\033[92mACESAlgebra.add(a, b): `a` does not need to be refreshed as refresh would increase saturation by \u0394 = +{round(100 * (estimated_refresh_noise - a.lvl) / total_noise, 4)}%\033[0m")
                    a_refreshed = a
                    ...
                    if self.debug and not refreshable:
                        print("\033[92mACESAlgebra.add(a, b): `a` is not refreshable!\033[0m")
                    ...
                    if a_ is not None:
                        if self.debug:
                            print("\033[92mACESAlgebra.add(a, b): `a` is now refreshable.\033[0m")
                        a_refreshed = self.refresh(refresher=self.refresher, corefresher=a_.corefresher(self.encrypter))
                        if self.debug:
                            sat_evolution = f"{round(100 * a.lvl / total_noise, 4)}% -> {round(100 * a_.lvl / total_noise, 4)}% -> {round(100 * a_refreshed.lvl / total_noise, 4)}%"
                            print(f"\033[92mACESAlgebra.add(a, b): `a` was successfully refreshed: {sat_evolution}.\033[0m")
                    else:
                        a_refreshed = self.refresh(refresher=self.refresher, corefresher=a.corefresher(self.encrypter))
                        if self.debug:
                            sat_evolution = f"{round(100 * a.lvl / total_noise, 4)}% -> {round(100 * a_refreshed.lvl / total_noise, 4)}%"
                            print(f"\033[92mACESAlgebra.add(a, b): `a` was successfully refreshed: {sat_evolution}.\033[0m")

                # Refresh `b` if needed
                estimated_refresh_noise = self.assess_refresh_level(refresher=self.refresher, corefresher=b.corefresher(self.encrypter))
                if estimated_refresh_noise >= b.lvl:

                    if self.debug:
                        print(f"\033[92mACESAlgebra.add(a, b): `b` does not need to be refreshed as refresh would increase saturation by \u0394 = +{round(100 * (estimated_refresh_noise - b.lvl) / total_noise, 4)}%\033[0m")
                    b_refreshed = b
                    ...
                    if self.debug and not refreshable:
                        print("\033[92mACESAlgebra.add(a, b): `b` is not refreshable!\033[0m")
                    ...
                    if b_ is not None:
                        if self.debug:
                            print("\033[92mACESAlgebra.add(a, b): `b` is now refreshable.\033[0m")
                        b_refreshed = self.refresh(refresher=self.refresher, corefresher=b_.corefresher(self.encrypter))
                        if self.debug:
                            sat_evolution = f"{round(100 * b.lvl / total_noise, 4)}% -> {round(100 * b_.lvl / total_noise, 4)}% -> {round(100 * b_refreshed.lvl / total_noise, 4)}%"
                            print(f"\033[92mACESAlgebra.add(a, b): `b` was successfully refreshed: {sat_evolution}.\033[0m")
                    else:
                        b_refreshed = self.refresh(refresher=self.refresher, corefresher=b.corefresher(self.encrypter))
                        if self.debug:
                ...
                if self.debug:
                    ...
                    print(f"\033[94mACESAlgebra.add: refresh operation is to decrease {sat_delta}.\033[0m")

                if refreshed_noise / total_noise >= .99 :
                    raise Exception(f"ACESAlgebra.add: refresh operation failed due to saturation at {round(100 * refreshed_noise / total_noise, 4)}%")

                return self.add(a_refreshed if a.lvl > a_refreshed.lvl else a,
        ...
        if self.debug:
            ...
            print(f"\033[93mWarning in ACESAlgebra.add: saturation increasing from {input_saturation} to {output_saturation} of max. noise level.\033[0m")
    ...
    def mult(self, a: ACESCipher, b: ACESCipher, refresh: bool = True) -> ACESCipher:
        ...
        if noise / total_noise >= .99:
            ...
            if self.debug:
                print(f"\033[95mACESAlgebra.mult: next operation will reach max. saturation at {round(100 * noise / total_noise, 4)}% of max. noise level.\033[0m")
            ...
            if refresh and self.refresh_classifier is not None and self.encrypter is not None and self.refresher is not None:
                ...
                if estimated_refresh_noise >= a.lvl:
                    ...
                    if self.debug:
                        print(f"\033[92mACESAlgebra.mult(a, b): `a` does not need to be refreshed as refresh would increase saturation by \u0394 = +{round(100 * (estimated_refresh_noise - a.lvl) / total_noise, 4)}%\033[0m")
                    a_refreshed = a
                    ...
                    refreshable = self.refresh_classifier(a)
                    if self.debug and not refreshable:
                        print("\033[92mACESAlgebra.mult(a, b): `a` is not refreshable!\033[0m")
                    ...
                    if a_ is not None:
                        if self.debug:
                            print("\033[92mACESAlgebra.mult(a, b): `a` is now refreshable.\033[0m")
                        a_refreshed = self.refresh(refresher=self.refresher, corefresher=a_.corefresher(self.encrypter))
                        if self.debug:
                            sat_evolution = f"{round(100 * a.lvl / total_noise, 4)}% -> {round(100 * a_.lvl / total_noise, 4)}% -> {round(100 * a_refreshed.lvl / total_noise, 4)}%"
                            print(f"\033[92mACESAlgebra.mult(a, b): `a` was successfully refreshed: {sat_evolution}.\033[0m")   
                    else:
                        a_refreshed = self.refresh(refresher=self.refresher, corefresher=a.corefresher(self.encrypter))
                        if self.debug:
                            sat_evolution = f"{round(100 * a.lvl / total_noise, 4)}% -> {round(100 * a_refreshed.lvl / total_noise, 4)}%"
                ...
                if estimated_refresh_noise >= b.lvl:
                    ...
                    if self.debug:
                        print(f"\033[92mACESAlgebra.mult(a, b): `b` does not need to be refreshed as refresh would increase saturation by \u0394 = +{round(100 * (estimated_refresh_noise - b.lvl) / total_noise, 4)}%\033[0m")
                    b_refreshed = b
                    ...
                    refreshable = self.refresh_classifier(b)
                    if self.debug and not refreshable:
                        print("\033[92mACESAlgebra.mult(a, b): `b` is not refreshable!\033[0m")
                    ...
                    if b_ is not None:
                        if self.debug:
                            print("\033[92mACESAlgebra.mult(a, b): `b` is now refreshable.\033[0m")
                        b_refreshed = self.refresh(refresher=self.refresher, corefresher=b_.corefresher(self.encrypter))
                        if self.debug:
                            sat_evolution = f"{round(100 * b.lvl / total_noise, 4)}% -> {round(100 * b_.lvl / total_noise, 4)}% -> {round(100 * b_refreshed.lvl / total_noise, 4)}%"
                            print(f"\033[92mACESAlgebra.mult(a, b): `b` was successfully refreshed: {sat_evolution}.\033[0m")
                    else:
                        b_refreshed = self.refresh(refresher=self.refresher, corefresher=b.corefresher(self.encrypter))
                        if self.debug:
                            sat_evolution = f"{round(100 * b.lvl / total_noise, 4)}% -> {round(100 * b_refreshed.lvl / total_noise, 4)}%"
                ...
                a_lvl = min(a.lvl, a_refreshed.lvl)
                b_lvl = min(b.lvl, b_refreshed.lvl)
                refreshed_noise = (a_lvl + b_lvl + a_lvl * b_lvl ) * self.p + ((self.p - 1)**2) // self.p

                if self.debug:
                    sat_delta = f"saturation of {round(100 * noise / total_noise, 4)}% by \u0394 = {round(100 * (refreshed_noise - noise) / total_noise, 4)}%"
                    print(f"\033[94mACESAlgebra.mult: refresh operation is to decrease {sat_delta}.\033[0m")

                if refreshed_noise / total_noise >= .99 :
                    raise Exception(f"ACESAlgebra.mult: refresh operation failed due to saturation at {round(100 * refreshed_noise / total_noise, 4)}%")

                return self.mult(   a_refreshed if a.lvl > a_refreshed.lvl else a,
                                    b_refreshed if b.lvl > b_refreshed.lvl else b, 
                                    refresh = False)
        ...
        if self.debug:
            ...
            input_saturation    = f"({round(100 * a.lvl / total_noise, 4)}%, {round(100 * b.lvl / total_noise, 4)}%)"
            output_saturation   = f"{round(100 * noise / total_noise, 4)}%"
            print(f"\033[93mWarning in ACESAlgebra.mult: saturation increasing from {input_saturation} to {output_saturation} of max. noise level.\033[0m")
...
class ACESRefreshClassifier(object):
    ...
    def refresh_classifier(self, ciphertext: Union[ACESCipher, ACESPseudoCipher]) -> bool:
        ...
        if self.debug:
            print("\t - margin condition satisfied:", margin_condition, "| margin:", margin)
            print("\t - is locator", is_locator)
            if not(margin_condition) or not(is_locator):
                print("\033[91m\t - actual refreshability:", self.is_refreshable(ciphertext), "\033[0m")

        return margin_condition and is_locator
    
    def find_affine(self, 
                    search_min: int, 
                    search_max: int,
                    training_epochs: int,
                    name: str = "default", 
                    file_option: Optional[str] = None) -> dict[str, Union[list[Tuple[list[int], float]], int]]:
        ...
        if file_option is not None:
            ...
            if file_option == "pickle":
                ...
                print(f"ACESRefreshClassifier.construct: Extension {file_option} non supported")
                return None
...
def refresh_classifier_generator(locators: list[Tuple[list[int], float]], 
                                 directors: list[Tuple[list[int], float]], 
                                 p: int, 
                                 q: int, 
                                 **args) -> Callable[[Union[ACESCipher, ACESPseudoCipher]], bool]:

    def refresh_classifier(ciphertext: Union[ACESCipher, ACESPseudoCipher]) -> bool:
        
        vector = np.array([d(arg=1) if isinstance(d, Polynomial) else d for d in ciphertext.dec], dtype=int)
        margin = []
        ...
        while max(already_explored_vs_counts.values()) < len(vector) :
            ...
            sorted_indices = non_zero_indices[np.lexsort((-np.abs(vector[non_zero_indices]), vector[non_zero_indices]))][::-1]
            
            # Pick the first index that is not in the last indicies of previously_explored
            i = next((idx for idx in sorted_indices if idx not in already_explored_indices[-1:]), None)
            ...
            for j, (v, f) in enumerate(directors):
                ...
                if v[i] != 0 and v[i] == np.max(v):
                    if (best_v is None or abs(v[i]) < abs(best_v[i]) or (abs(v[i]) == abs(best_v[i]) and np.count_nonzero(v) < np.count_nonzero(best_v))):
        ...
        for v0, f0 in locators:
            ...
            if np.array_equal(vector, v0_):
                ...
                return (margin_modulo < 1) and margin_condition and all(iota_homomorphism_condition)
